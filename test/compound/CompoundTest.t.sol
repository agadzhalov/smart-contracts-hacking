...
import "src/compound/CompoundUser.sol";

/**
@dev run "forge test --fork-url $ETH_RPC_URL --fork-block-number 16776127 --match-contract MM2 -vvv" 
*/
contract CompoundTest is Test {
    address constant COMPOUND_COMPTROLLER = address(0x3d9819210A31b4961b30EF54bE2aeD79B9c9Cd3B);
    ...
    address constant C_USDC = address(0x39AA39c021dfbaE8faC545936693aC917d5E7563);
    // AAVE DAI Variable Debt Token
    address constant C_DAI = address(0x5d3a536E4D6DbD6114cc1Ead35777bAB948E3643);
    ...
    function test_Attack() public {
        ...
        usdc.approve(address(compUser), AMOUNT_TO_DEPOSIT);
        ...
        uint cUSDCBalanceBefore = cUsdc.balanceOf(address(compUser));
        ...
        assertEq(cUsdc.balanceOf(address(compUser)) < cUSDCBalanceBefore / 100, true);
